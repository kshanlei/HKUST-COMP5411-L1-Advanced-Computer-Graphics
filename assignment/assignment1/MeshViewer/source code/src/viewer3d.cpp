...
void ViewerData::updateVertexInfo() {
	...
	if (mMesh->isVertexNormalDirty()) {
		...
		std::cout << "Scale: " << scale.norm() << std::endl;
...
int Viewer3D::launchInit() {
	...
	mNGui->addVariable< int >("Iter. Num.", [&](int num)
	{
		mMesh->setiternum(num);
	}, [&]()
	{
		return 0;
	});

	mNGui->addVariable< float >("lambda", [&](float val)
	{
		mMesh->setlambda(val);
	}, [&]()
	{
	...
	mNGui->addButton("Update", [&]() { 
		std::cout << "Lambda value: " << mMesh->readlambda() << std::endl;
		std::cout << "Num of iterations: " << mMesh->readiternum() << std::endl;
		for (int i = 0; i < mMesh->readiternum(); i++)
		{
			smoothMesh();
		}
	});
...
void Viewer3D::smoothMesh() {
	...
	case SmoothScheme::Implicit:
		mMesh->implicitSmooth(mUseCotWeights);
		break;
	case SmoothScheme::Explicit:
	default:
		mMesh->explicitSmooth(mUseCotWeights);
		break;
	}
}

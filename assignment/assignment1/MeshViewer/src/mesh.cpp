...
void Mesh::implicitSmooth(bool cotangentWeights) {
	...
	if (cotangentWeights) {
		...
		int L = (int)mVertexList.size();
		Eigen::VectorXf b(3 * L);
		int nv = mVertexList.size();
		for(int i = 0; i < nv; i++) {
			Vertex* curVertex = mVertexList[i];
			//put the position of each Vertex into one demension array
			for (int i=0; i<3; i++) {
		...
		for(int i=0; i< 3 * L; i++) {
			tripletList.push_back(TRIPLET(i, i, 1 + lambda));
		}
		for(int j = 0; j < nv; j++) {
			Vertex* curVertex = mVertexList[j];
			//find all the outbound edge of the current vertex
			...
			for(int k = 0; k < outBoundHEdges.size(); k++) {	
				//calculate cot(apha)
				HEdge* outBoundHEdge = outBoundHEdges[k];				
				...
				float cot1 = halfEdge11.dot(halfEdge12) / halfEdge11.cross(halfEdge12).norm();
				...
				float cot2 = halfEdge21.dot(halfEdge22) / halfEdge21.cross(halfEdge22).norm();
			...
			//calculate the Triplet for each edge 
			for(int m = 0; m < outBoundHEdges.size(); m++) {
				HEdge* outBoundHEdge = outBoundHEdges[m];
				float weight = weights[m];
				int nextIndex = outBoundHEdge->end()->index();
				for (int n = 0; n < 3; n++) {
					tripletList.push_back(TRIPLET(curIndex * 3 + n, nextIndex * 3 + n, -lambda * weight / sumWeights));
				}
			}
			
		}
		
		// copy the TRIPLET vector to A
		Eigen::SparseMatrix< float > A(3 * L, 3 * L);
		A.setFromTriplets(tripletList.begin(), tripletList.end());
		
		// declaere a solver matrix x
		Eigen::VectorXf x(3 * L);
		for (int i=0; i<3 * L; i++) {
			x[i] = 0.0;
		}
		//calculate the x for the given A and b
		fnConjugateGradient(A, b, 10, 1e-12, x);

		for(int i = 0; i < nv; i++) {
			Vertex* curVertex = mVertexList[i];
			Eigen::Vector3f newPosition;
			for (int j = 0; j < 3; j++) {
				newPosition[j] = x[curVertex->index() * 3 + j];
			}
		...
		for(int j = 0; j < mVertexList.size(); j++) {
			...
			// for each vertex, calculate the TRIPLET through all its outbound edge
			for(int k = 0; k < outBoundHEdges.size(); k++) {
				HEdge* outBoundHEdge = outBoundHEdges[k];	
				int nextIndex = outBoundHEdge->end()->index();
				for (int i=0; i<3; i++) {
					tripletList.push_back(TRIPLET(curIndex * 3 + i, nextIndex * 3 + i, -lambda / outBoundHEdges.size()));
				}
			}
		}
